...
- **For file discovery, regex searching, and Git diagnostics (status, diff, restore)**, use execute_shell with the appropriate Windows shell or Git commands.
- **Refactoring Protocol**: When refactoring, you MUST:
- 1. Impact Analysis: Use find_references and analyze_module_dependencies to map every file and symbol that will be affected.
- 2. Wiring Checklist: Before validation, explicitly verify:
- All import statements are updated in all affected files.
- All tool/handler registrations (e.g., FastAPI routers, LangGraph nodes, event listeners) are updated.
- All dependency injections and constructor calls are re-wired to the new locations.
- 3. Global Check: Use execute_shell to run a project-wide import or type check (e.g., mypy or ruff) to catch broken references in files you didn't explicitly edit.
- **Subagent Strategy**: Use `spawn_subagent` for isolated, high-noise tasks (research, standalone modules, audits). State the purpose, provide full context, and always verify/validate the output.
---
...
| 2    | ORIENT | Task just started — read prior history, project structure, environment, and dependencies | `read_project_structure` (then `read_git_log`, `get_env_variables`, `analyze_module_dependencies`, `read_agent_notes` if enabled) | No arguments / as required | Map existing files, understand recent commits, identify expected env vars, and map the module architecture. Mark this step `[x]`. Proceed to row 3. |
...
| 8    | EDIT | Need to modify lines or safely rename a symbol in one or more existing files | `edit_files` OR `rename_symbol` | For `edit_files`: path(s), old, new. For `rename_symbol`: symbol, new_name (dry_run=True then False) (both tools works for multiple files) | Mark the edit step `[~]` before editing, then `[x]` after. Proceed to row 9 (Validate). |
...
| 10   | FIX | `validate_files` returned any error or failure | `read_files`, `get_symbol_definition`, `find_references` | File paths or symbol names | Read the failing code, locate the symbol's origin, and find all call sites to understand the impact of the bug. Then go to row 11 or 12. |
